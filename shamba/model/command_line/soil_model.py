...
class RothC(object):
    ...
    def get_rmf(self):
        ...
        deficit = self.climate.rain - self.climate.evaporation
        ...
        deficit = self.climate.rain - self.climate.evaporation * 0.75

        # Loop through each month
        for i in range(12):
            accTsmd = self._get_acc_tsmd(accTsmd, deficit[m], self.cover[m], max)
        ...
        # Temperature RMF (a)
        a = np.zeros(12)
        for i in np.where(self.climate.temperature > -5.0):
            a[i] = 47.91 / (1.0 + np.exp(106.06 / (self.climate.temperature + 18.27)))
...
class ForwardRothC(RothC):
    ...
    def solver(self, Ci, crop=[], tree=[], litter=[], fire=[], solveToValue=False):
        ...
        for i in range(1, configuration.N_YEARS + 1):
            ...
            Ctemp = integrate.odeint(
                self.dC_dt, C[i - 1], t, args=(x[i - 1], self.k, inputs[i - 1].sum())
            )
