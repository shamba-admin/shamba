...
class TreeGrowth(object):
    ...
    def __init__(self, tree_params, growth_params, allom="chave dry"):
        ...
        if "biomass" not in growth_params:
            ...
            try:
                for i in range(len(self.diam)):
                    self.biomass[i] = allometric[self.allom](self.diam[i], tree_params)
            except KeyError:
        ...
        self.funcDeriv = {
            "exp": self.exp_fn_deriv,
            "hyp": self.hyp_fn_deriv,
            "lin": self.lin_fn_deriv,
            "log": self.log_fn_deriv,
        }
    ...
    @classmethod
    def from_arrays(cls, tree, age, data, isBiomass=False, allom="chave dry"):
    ...
    def fit(self):
        ...
        for curve in self.func:
            # Do the fitting
            try:
                fitParams = optimize.curve_fit(
                    self.func[curve], self.age, self.biomass, init[curve]
                )
                par = fitParams[0]
                params[curve] = par
            ...
            if curve == "log":
                if fitParams is not None:
                    data[curve] = self.func[curve](self.age, par[0], par[1], par[2])
                else:
                ...
                if fitParams is not None:
                    data[curve] = self.func[curve](self.age, par[0], par[1])
                else:
    ...
    def plot_(self, fit=True, saveName=None):
        ...
        if fit:
            for curve in self.allFitData:
                ax.plot(self.age, self.allFitData[curve], "-", label="%s fit" % curve)
    ...
    def print_(self, fit=None, params=None, mse=None):
        ...
        if params and mse:
            ...
            print(
                (
                    "b          -     %6.2f    -      %6.2f"
                    % (self.allFitParams["hyp"][1], self.allFitParams["log"][1])
                )
            )
            print(
                (
    ...
    def save_(self, file="tree_growth.csv"):
        ...
        csv_handler.print_csv(
            file,
            np.column_stack((self.age, self.diam, self.biomass)),
