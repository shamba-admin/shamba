...
def get_arguments_interactively():
    ...
    print("1 - Info")
    print("2 - Debug")
    arguments['verbose'] = int(input("Enter verbosity level (0/1/2): ").lower().startswith('0'))
    ...
    # Prompt for report
    arguments['report'] = input("Print report to stdout? (y/n): ").lower().startswith('y')

    # Prompt for graph
    arguments['graph'] = input("Show plots? (y/n): ").lower().startswith('y')

    # Add some instructions here
    print("""Complete in full the Excel worksheet 'SHAMBA input output template v1',
    ...
    arguments['input-file-name'] = input_file_name if input_file_name else 'WL_input.csv'
